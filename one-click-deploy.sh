...
echo "下一步："
echo "  1. 登录管理后台修改密码"
echo "  2. 在后台「令牌」页面创建管理员令牌"
echo "  3. 运行 sync-free-keys.sh 自动导入免费 Key"
echo "  4. 在后台「令牌」页面创建统一对外 Key"
echo "  5. 开启用户注册：系统设置 → 通用 → 允许注册"
echo ""
echo "自动同步免费 Key："
echo "  export ADMIN_TOKEN=你的管理员令牌"
echo "  ./sync-free-keys.sh"
echo ""
echo "定时同步（每6小时）："
echo "  (crontab -l 2>/dev/null; echo '0 */6 * * * cd /root/new-api && ADMIN_TOKEN=你的令牌 ./sync-free-keys.sh') | crontab -"
echo "============================================"
